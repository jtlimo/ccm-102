...
\begin{comment}
  Falar o que é Spoofing de GPS antes de falar dos tipos. Bem rapidinho.
\end{comment}

% Os ataques de \textit{spoofing} de GPS
% são classificados em: simples, intermediários e sofisticados \cite{Aissou2021}.
...
sendo eles: \textit{Naive Bayes}, \textit{K-NN},
...
sendo eles: (iii) \textit{Naive Bayes}, (iv) \textit{K-NN} e
(v) \textit{Gaussian Naive Bayes}.

A proposta de mudança deste artigo consiste em avaliar outros modelos
para a classificação de possíveis ataques de \textit{spoofing} 
a sinais de GPS, com enfoque em algoritmos
que não são baseados em árvore já citados.
No entanto com base no desempenho do \textit{Random Forest},
será feita a implementação do algoritmo 
(vi) \textit{Extremely randomized trees}, um modelo melhorado
