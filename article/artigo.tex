...
Random Forest, XGBoost, Naive Bayes, K-NN,
...
que utilizou a implementação de algoritmos como: 
...
a sinais de GPS, com enfoque em algoritmos
que não são baseados em árvore, sendo estes:
(i) \textit{Naive Bayes}, (ii) \textit{K-NN}
e (iii) \textit{Gaussian Naive Bayes}.

Além da reimplementação do algoritmo (iv) \textit{Random Forest}
que foi utilizado no artigo \cite{Aissou2021}
e obteve o pior desempenho no comparativo.
Com base no desempenho do \textit{Random Forest},
será feita a implementação do algoritmo 
(v) \textit{Extremely randomized trees}, um modelo melhorado
do \textit{Random Forest}, que se diferem pelo fato
...
Também será feita a reimplementação do algoritmo
(v) \textit{XGBoost} que foi utilizado no artigo \cite{Aissou2021}
e obteve o melhor desempenho no comparativo dos modelos baseados em árvore.
