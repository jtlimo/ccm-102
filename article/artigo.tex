...
melhorar o desempenho do processamento e o
\textit{Random Under Sampler} para reduzir o desbalanceamento das classes
que dificultam o treinamento e a análise dos dados.

Os motivos pelos quais foram escolhidos esses métodos estão embasados nos seguintes artigos: 
\begin{itemize}
  \item \textbf{PCA:} Nessa pesquisa de \cite {Zhang2019MachineLW} 
    foi utilizada para detectar \textit{malwares}
    e constatou a remoção de redundâncias dos dados com perda mínima.
  \item \textbf{Standard Scaler:} Nesse artigo  de \cite {DEAMORIM2023109924} 
    a técnica foi empregada para verificar 
    a performance dos algoritmos de classificação e provou ser eficaz.
  \item \textbf{Random Under Sampler:} Nesse artigo de \cite {DEAMORIM2023109924} 
    desempenhou papel crucial para a detecção de intrusão em \textit{smartphones}, 
    pois o modelo é bem empregado quando há dados desbalanceados.
\end{itemize}
